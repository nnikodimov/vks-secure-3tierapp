...
- [terraform/antrea-coredns/](terraform/antrea-coredns/) - fully standalone:
  ...
  depended on by, `baseline`/`antrea-policy`/`database-policy`.
...
makes `baseline` apply first. All four modules inherit the root's provider
configuration automatically - none declares its own `provider` block, so none
of them can be `init`/`apply`'d from inside its own folder; always run
...
terraform apply -target=module.antrea_coredns   # independent - any point works
